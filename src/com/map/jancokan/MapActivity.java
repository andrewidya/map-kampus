...
public class MapActivity extends android.support.v4.app.FragmentActivity {
	...
	private void drawPosisionOnMap(Location location){
		...
		try {
			List<Address> addressesNames = geocoder.getFromLocationName(message, 3);
			if(addressesNames.size() > 0) {
				...
				map.addMarker(new MarkerOptions().position(new LatLng(addressesNames.get(0).getLatitude(), addressesNames.get(0).getLongitude())).title(nameCoordinate)).showInfoWindow();
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
