...
public class NavigateActivity extends android.support.v4.app.FragmentActivity implements OnMyLocationChangeListener {
	...
	public class CurrentLocationProvider implements LocationListener, LocationSource {
		...
		public void activate(OnLocationChangedListener listener) {
			...
			// To check the provider available on device, then get location update
			if(gpsEnabled) {
		...
		public void onLocationChanged(Location location) {
			...
			if(listener != null) {
				...
				if(location != null) {
					url = makeURL(location.getLatitude(), location.getLongitude(), coordinate.latitude, coordinate.longitude);
	...
	public void drawPath(String result) {
	    ...
	    try {
	           ...
	           for(int z = 0; z<list.size()-1;z++){
	                ...
	                lineOptions.add(new LatLng(src.latitude, src.longitude), new LatLng(dest.latitude, dest.longitude)).width(2).color(Color.BLUE).geodesic(true);
	                Polyline line = map.addPolyline(lineOptions);// NavigateActivity.this.map.addPolyline(new PolylineOptions().add(new LatLng(src.latitude, src.longitude), new LatLng(dest.latitude, dest.longitude)).width(2).color(Color.BLUE).geodesic(true));
	                line.setPoints(list);
	           }
